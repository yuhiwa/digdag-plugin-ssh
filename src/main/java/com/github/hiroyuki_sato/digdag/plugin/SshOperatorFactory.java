    ...
    private class SshOperator extends BaseOperator {
        ...
        public TaskResult runTask() {
            ...
            try {
                ...
                RetryExecutor retryExecutor = retryExecutor().retryIf(exception -> true)
                        .withInitialRetryWait(initial_retry_wait).withMaxRetryWait(max_retry_wait)
                        .onRetry((exception, retryCount, retryLimit, retryWait) -> logger.warn(
                                "Connection failed: retry {} of {} (wait {}ms)", retryCount, retryLimit, retryWait,
                                exception))
        ...
        private void setupHostKeyVerifier(Session session) {
            ...
            /*
             * ssh.loadKnownHosts(); } catch (IOException ex) { throw
